...
parser.add_argument("--mark_version", type=str, default=None, 
                    help="모델 버전 (예: mark4.6). 이 버전에 따라 입/출력 폴더가 결정됩니다.")
...
def process_all(mark_version):
    ...
    if mark_version is None:
        print("[CRITICAL ERROR] --mark_version 인자가 반드시 필요. (예: --mark_version mark4.6)")
    ...
    if not os.path.isdir(data_root):
        ...
        print("폴더 구조가 '.../mark4.6/data/{train|val|test}' 형태인지 확인해주세요.")
        sys.exit(1)

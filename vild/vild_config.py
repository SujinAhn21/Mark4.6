# vild_config.py(Mark4.2, Mark4.6)
...
class AudioViLDConfig:
    ...
    주요 추가 사항
    - class_weight: CrossEntropyLoss의 클래스 가중치 리스트(클래스 순서와 동일)
      예) classes == ["dragging", "others"]일 때 [w_dragging, w_others]
      기본은 None. mark4.2/4.6에서는 권장 초기값 [1.0, 2.0]을 제공(필요 시 변경).
    - get_class_weight_tensor(device, dtype): 손실 계산 시 바로 넘길 수 있는 텐서 반환
    """
    
    def __init__(self, mark_version="mark4.6", # change needed
        ...
        ### CE vs 임베딩 정합 신호의 비율(증류 α에 해당)
        self.text_loss_weight = 0.7  # 원래 1.0 | 현재 0.7로 시도중
        self.image_loss_weight = 0.3  # 원래 1.0 | 현재 0.3으로 시도중.
        ...
        # === CrossEntropy class_weight 설정 ===
        # 직접 인자로 전달한 경우(우선됨)
        if class_weight is not None:
            self.class_weight = self.validate_class_weight(class_weight)
        else:
            # 기본값(권장 초기값)
            self.class_weight = self._default_class_weight_for_version()
            
    
    def _default_class_weight_for_version(self) -> Optional[list]:
        # mark4.2, mark4.6에 한해 기본 가중치 적용.
        # class 인덱스 순서와 동일히야 하므로 반드시 현재 self.classes를 기준으로 반환.
        
        if self.mark_version in {"mark4.2", "mark4.6"}:
            # ["dragging","others"] 또는 ["water_toilet","others"] ==> [1.0, 2.0]
            return [1.0, 2.0]
        return None # 그 외 버전은 미적용인데 사실 당연한게 어차피 이 스크립트는 여기서만 씀.
    
    def _validate_class_weight(self, weight: Sequence[float]) -> list:
        w = list(weight)
        if len(w) != len(self.labeled_classes):
            raise ValueError(
                f"class_weight 길이({len(w)})가 클래스 수({len(self.labeled_classes)})와 다릅니다. "
                f"순서는 self.classes와 동일해야 합니다: {self.labeled_classes}"
            )
        if any(v <= 0 for v in w):
            raise ValueError("class_weight의 모든 값은 양수여야 합니다.")
        return w
    
    def get_class_weight_tensor(self, device=None, dtype=torch.float):
        """
        손실 계산 시 넘길 weight 텐서를 반환.
        device가 주어지면 해당 디바이스로 이동.
        """
        if self.class_weight is None:
            return None
        t = torch.tensor(self.class_weight, dtype=dtype)
